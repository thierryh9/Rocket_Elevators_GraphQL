...
async function getEverything({email}){
    ...
    async function updatecreated({customer_id, building_id, battery_id, column_id, elevator_id, report}) {
        ...
        // Query the MySQL batteries table.
            creation_of_intervention = await query("INSERT INTO interventions (customer_id, building_id, battery_id, column_id, elevator_id, result, report, status, created_at, updated_at ) VALUES(1,'"+customer_id+"','"+building_id+"','"+battery_id+"','"+column_id+"','"+elevator_id+"', 'incomplete','"+report+"', 'pending', CURRENT_TIMESTAMP(), CURRENT_TIMESTAMP());");
